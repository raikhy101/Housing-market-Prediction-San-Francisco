...
main <- read.csv(file = "C://Users//Public//Jacksonville_Master.csv")
...
summary(lmodel)
#running the linear model over the entire timeframe shows that
...
acf(diffres)
pacf(diffres)
Box.test(residuals(lmodel))
dynamic = Arima(y = main[,2],order = c(0,1,0),xreg = main[,3:8])
h = 50 
