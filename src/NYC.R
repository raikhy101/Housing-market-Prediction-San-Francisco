...
main <- read.csv(file = "C://Users//Public//NYC_Master.csv")
...
name = colnames(main)
main = data.frame(main,def)
colnames(main) = c(name,"Default")
...
pacf(diffres)
Box.test(diffres)
dynamic = Arima(y = main[,2],order = c(1,1,1),xreg = main[,3:8])
h = 50 #how many periods to forecast
#change the numbers to numbers you want to have
m = auto.arima(main[,3])
